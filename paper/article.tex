...
Across four task types, MetaGlyph achieves 62--81\% fewer tokens while preserving meaning, though reliability varies by symbol. The membership operator ($\in$) works best (up to 33.3\% fidelity), while complex operators like implication show zero fidelity without explicit explanation. These results show that symbolic instruction compression is viable, but success depends critically on which symbols are used.
\end{abstract}
...
OLMo 3 7B & ---\textsuperscript{*} & ---\textsuperscript{*} & ---\textsuperscript{*} & ---\textsuperscript{*} & ---\textsuperscript{*} \\
Kimi K2 & ---\textsuperscript{*} & ---\textsuperscript{*} & ---\textsuperscript{*} & ---\textsuperscript{*} & ---\textsuperscript{*} \\
\bottomrule
\multicolumn{6}{l}{\textsuperscript{*}\footnotesize Results pending; experiments in progress.}
\end{tabular}
\end{table}
...
\midrule
$\in$ (membership) & 33.3\% & 0.0\% & \textbf{18.9\%} & ---\textsuperscript{*} & ---\textsuperscript{*} \\
$\to$ (transformation) & 0.0\% & 0.0\% & 0.0\% & ---\textsuperscript{*} & ---\textsuperscript{*} \\
$\cap$ (intersection) & 0.0\% & 0.0\% & 0.0\% & ---\textsuperscript{*} & ---\textsuperscript{*} \\
$\Rightarrow$ (implication) & 0.0\% & 0.0\% & 0.0\% & ---\textsuperscript{*} & ---\textsuperscript{*} \\
\bottomrule
\multicolumn{6}{l}{\textsuperscript{*}\footnotesize Results pending.}
\end{tabular}
\end{table}

Only $\in$ (membership) shows meaningful fidelity, up to 33\%. Complex operators like $\Rightarrow$ (implication) and $\cap$ (intersection) show zero fidelity, indicating models do not reliably interpret them as instruction primitives. Surprisingly, the largest model (Gemma 12B) shows 0\% fidelity on all operators, suggesting instruction tuning may create stronger natural-language biases.
...
The problem is that in math texts, $\cap$ connects \emph{sets}, whereas here it connects \emph{predicates}. This is a generalization models have not learned. Similarly, $\Rightarrow$ appears in diverse contexts with varying interpretations: mathematical proofs, programming, logical specifications. Without consistent usage, models do not reliably treat it as ``if-then.''

\subsection{Model Scale Effects}

Larger models parse better (Gemma 12B: 100\% JSON success vs.\ Llama 3B: 30\%). But larger models show \emph{worse} operator fidelity (Gemma: 0\% vs.\ Llama: 33\% on $\in$). Instruction tuning may create stronger natural-language biases, making models treat symbols as formatting rather than meaning.
...
Several practical lessons emerge from this preliminary experiment. Compression works: 62--81\% token reduction is achievable. However, practitioners should use larger models, as small models (3B) struggle to parse symbolic prompts. Operators must be chosen carefully, since $\in$ (membership) partially works while $\Rightarrow$ and $\cap$ do not. Hybrid prompts may offer the best tradeoff, using symbols for structure and natural language for critical constraints.

\subsection{What We Showed}

Symbolic compression preserves meaning up to 44\% of the time for extraction tasks. The 0\% CTRL equivalence confirms models respond to symbolic \emph{meaning}, not just appearance. Operator reliability varies: $\in$ works partially while complex operators do not.
...
This work establishes a baseline rather than characterizing limits. The experiment was intentionally narrow: four task families, three models with complete results, and a single symbolic language design. Many questions remain open.
...
The answer is nuanced, and two questions must be separated. First, does compression preserve meaning? Partially. Up to 44\% of the time, symbolic and prose instructions produce identical outputs, and the 0\% CTRL equivalence confirms this reflects genuine semantic understanding rather than superficial pattern matching. Second, which symbols work? Simple ones. Membership ($\in$) shows up to 33\% fidelity, while complex operators like implication ($\Rightarrow$) and conjunction ($\cap$) show zero fidelity. Models do not reliably interpret these as instructions without additional context.

The practical implications are straightforward. Symbolic compression achieving 62--81\% fewer tokens is achievable, but practitioners should not assume compressed prompts preserve meaning without testing. Hybrid approaches combining symbols for structure with natural language for critical constraints may work best.

Symbols that appear consistently in training data ($\in$) show partial stability, while symbols with varied usage ($\Rightarrow$) do not transfer reliably. As models scale and training data includes more structured content, this gap may narrow. This work provides an initial foundation for developing symbolic instruction languages that realize the efficiency potential of compact, compositional prompts.
